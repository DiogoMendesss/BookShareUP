<?php
  ...
  function loginSuccess($up_number, $password) {
    global $bsup;
    $stmt = $bsup->prepare('SELECT * FROM User WHERE up_number = ? AND password = ?');
  ...
  function mock_loginSuccess($up_number) {
    global $bsup;
    $stmt = $bsup->prepare('SELECT * FROM User WHERE up_number = ?');
  ...
  function getUserFullName($up_number) {
    global $bsup;
    $stmt = $bsup->prepare('SELECT name FROM User WHERE up_number = ?');
    $stmt->execute(array($up_number));
  ...
  function insertUser($up_number, $password, $full_name, $faculty_campus) {
    global $bsup;
    $stmt = $bsup->prepare('INSERT INTO User (up_number, password, name) VALUES (?, ?, ?)');
